...
public class TranslationMovement : MonoBehaviour, IMovement
{
    ...
    public IEnumerator DoTranslationX(Transform target,float speed, Vector2 direction, float step)
    {
    ...
    public IEnumerator DoTranslationY(Transform target ,float speed, Vector2 direction)
    {
        Vector2 velocity = new Vector2(0, 0);
    ...
    public IEnumerator DoTranslationXY(float speed, Vector2 direction)
    {
        Vector2 velocity = new Vector2(0, 0);
        velocity.x = speed * Time.deltaTime;
        velocity.y = speed * Time.deltaTime;
        transform.Translate(new Vector2(direction.x * velocity.x, direction.y * velocity.y), Space.World);
        yield return null;
    }
    ...
    public IEnumerator DoTranslationOscillatingX(float frequency, float amplitude)
    {
        ...
        transform.Translate(new Vector2(velocity.x, velocity.y));
        yield return null;
    }
